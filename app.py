...
from ml_model import predict_contributions
...
@app.route('/predict_contributions/<username>', methods=['GET'])
def predict_user_contributions(username):
    try:
        predictions = predict_contributions(username)
        if "error" in predictions:
            return jsonify({'error': predictions["error"]}), 500
        
        return jsonify({
            'username': username,
            'predictions': predictions['weekly_predictions']
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
